...
export default function AdminLayout({ children }) {
  ...
  const navItems = [
    ...
    { name: 'Produk', icon: Folder, path: '/staff/produk' },
    { name: 'Laporan', icon: Calendar, path: '/staff/laporan' },
    { name: 'Profile', icon: Users, path: '/staff/profile' },
  ]
  ...
  useEffect(() => {
    ...
    if (session?.user?.role?.toLowerCase() !== 'staff gudang') return router.replace('/')
      ...
      <div className="absolute right-0 top-12 bg-white text-black rounded shadow-lg w-40 py-2 z-50">
        <button
          ...
          onClick={() => {
            ...
            router.push('/staff/profile') // arahkan ke halaman profil
          }}
        >
          Profil
        </button>
