...
import AdminLayout from '../../components/pimpinanlayout'
...
export default function AdminPage() {
  ...
  // ⛔ Jangan redirect jika status masih loading
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && session?.user?.role?.toLowerCase() === 'pimpinan') {
      fetchStats()
    }
  }, [session, status])
